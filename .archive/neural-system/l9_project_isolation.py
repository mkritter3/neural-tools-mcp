...
class L9ProjectIsolation:
    ...
    - Container named: l9-qdrant-{project_name}
    ...
    def get_project_context(self) -> Dict[str, str]:
        ...
        return {
            ...
            "path": project_path,
            "hash": project_hash,
            "container_name": f"l9-qdrant-{project_name}",
            "network_name": f"l9-network-{project_name}"
        }
        ...
        compose_config = {
            ...
            'services': {
                'qdrant': {
                    ...
                    'volumes': [
                        f"./{project['name']}_qdrant_storage:/qdrant/storage:z",
                        f"./{project['name']}_qdrant_snapshots:/qdrant/snapshots:z"
                    ],
                    'environment': [
                    ...
                    'labels': {
                        'l9.project': project['name'],
                        'l9.project.path': project['path'],
                        'l9.project.hash': project['hash'],
                        'l9.type': 'qdrant-db'
                    },
    ...
    def get_project_config(self) -> Dict[str, any]:
        ...
        return {
            ...
            'storage_path': f"{project['name']}_qdrant_storage",
            'snapshots_path': f"{project['name']}_qdrant_snapshots"
        }
    
    def list_all_project_containers(self):
        """List all L9 project containers"""
        containers = self.docker_client.containers.list(
            all=True,
            filters={'label': 'l9.type=qdrant-db'}
        )
        ...
        for container in containers:
            ...
            projects.append({
                'name': labels.get('l9.project', 'unknown'),
                'path': labels.get('l9.project.path', 'unknown'),
...
class L9SharedModelServer:
    ...
    def __init__(self):
        self.model_port = 8080  # Fixed port for model server
        self.container_name = "l9-shared-models"
        self.docker_client = docker.from_env()
    ...
    def generate_model_server_compose(self) -> str:
        ...
        compose_config = {
            ...
            'services': {
                'model-server': {
                    'image': 'l9-model-server:latest',
                    'container_name': self.container_name,
                    'ports': [
                        f"{self.model_port}:8080"  # Model serving API
                    ],
                    'environment': [
                        'MODEL_CACHE=/models',
                    ...
                    'volumes': [
                        'l9_model_cache:/models:z'  # Shared model cache
                    ],
                    'labels': {
                        'l9.type': 'model-server',
                        'l9.shared': 'true'
                    },
                    'restart': 'unless-stopped',
                    'healthcheck': {
                        'test': ['CMD', 'curl', '-f', f"http://localhost:8080/health"],
                        'interval': '30s',
                        'timeout': '10s',
                        'retries': 3
                    },
                    'networks': ['l9-shared-network'],
                    'deploy': {
                        'resources': {
                            'limits': {
                                'cpus': '2',
...
def main():
    ...
    print("🔍 L9 PROJECT ISOLATION MANAGER")
    print("=" * 60)
    ...
    print("  • Container named: l9-qdrant-{project_name}")
    print("  • Ports derived from project path hash")
    print("  • Data stored in project-specific volumes")
    print("  • Models shared via central server (planned)")
    
    # Save config for project
    config_path = Path(project['path']) / '.claude' / 'l9-project-config.json'
