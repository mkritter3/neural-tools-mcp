...
class L9ConfigManager:
    """
    Central configuration manager for L9 Neural System
    ...
    def _detect_and_configure(self) -> Dict:
        ...
        config_path = project_root / '.claude' / 'l9-project-config.json'
        ...
        config = {
            ...
            'storage_path': f'{project_name}_qdrant_storage',
            'snapshots_path': f'{project_name}_qdrant_snapshots',
            'model_server': {
                'host': 'localhost',
                'port': 8080,
                'shared': True
...
# Singleton instance
def get_config() -> L9ConfigManager:
    """Get the singleton config manager instance"""
    return L9ConfigManager()
