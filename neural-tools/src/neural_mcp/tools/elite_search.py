...
def _format_elite_results(
    ...
    for i, result in enumerate(results):
        ...
        formatted_result = {
            ...
            "graph_context": {
                "import_relevance": graph_context.get("import_relevance", 0),
                "call_depth": graph_context.get("call_depth", 0),
                "variable_usage": graph_context.get("variable_usage", 0),
                "class_usage": graph_context.get("class_usage", 0),
                "total_connections": sum([
                    graph_context.get("import_relevance", 0),
                    graph_context.get("call_depth", 0),
                    graph_context.get("variable_usage", 0),
                    graph_context.get("class_usage", 0)
                ])
            }
        }
        ...
        # Add related entities if present
        if graph_context.get("imports"):
            formatted_result["related_imports"] = [
                imp.get("path", "unknown")
                for imp in graph_context.get("imports", [])[:5]
            ]

        if graph_context.get("call_chain"):
            formatted_result["call_chain"] = [
                func.get("name", "unknown")
                for func in graph_context.get("call_chain", [])[:5]
            ]
