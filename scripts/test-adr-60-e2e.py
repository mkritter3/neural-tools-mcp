...
class ADR60TestSuite:
    ...
    async def test_concurrent_same_project(self) -> bool:
        ...
        tasks = [
            self.orchestrator.ensure_indexer(project_name, '/tmp/test')
            for _ in range(5)
        ]
    ...
    async def test_discovery_performance(self) -> bool:
        ...
        await self.orchestrator.ensure_indexer(project_name, '/tmp/test')
    ...
    async def test_redis_lock_prevents_duplicates(self) -> bool:
        ...
        result = await self.orchestrator.ensure_indexer(project_name, '/tmp/test')
        if result:
